...
public class superAgentMainPageController extends UIController{
    ...
    public void startAnimation() {
        ...
        reviewLabelButton.setOnMouseClicked(event -> {
        ...
        userSearchButton.setOnMouseClicked(event -> {
            System.out.println("stopping anim");
            if (event.getClickCount() == 1) {
                anim.stop();
                try {
                    setDisplayToSuperAgentSearchUsers();
                } catch (Exception e) {
                    e.printStackTrace();
                }
            }
        });
        createAgentButton.setOnMouseClicked(event -> {
            System.out.println("stopping anim");
            if (event.getClickCount() == 1) {
                anim.stop();
                try {
                    setDisplayToSuperAgentCreateAgent();
                } catch (Exception e) {
                    e.printStackTrace();
                }
            }
